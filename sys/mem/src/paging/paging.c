...
// Add more page tables to cover 32MB total (8 tables * 4MB)
// We already have 1, need 7 more.
uint32_t extra_page_tables[7][1024] __attribute__((aligned(4096)));
...
// frame variables
static uint32_t next_frame = 0x02000000; // Start allocation at 32MB mark to avoid overwriting kernel/heap
...
void paging_init(uint32_t memory_size) {
    ...
    // Map 0-32MB Identity
    for(int j=0; j<8; j++) {
        uint32_t* table = (j==0) ? first_page_table : extra_page_tables[j-1];
        uint32_t addr_start = j * 0x400000;
        
        for (unsigned int i = 0; i < 1024; i++) {
             table[i] = (addr_start + i * 0x1000) | 3;
        }
        page_directory[j] = ((unsigned int)table) | 3;
    }
